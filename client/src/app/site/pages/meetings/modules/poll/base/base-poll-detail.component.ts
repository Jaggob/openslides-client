    ...
    implements OnInit, OnDestroy
{
    ...
    private setEntitledUsersData(): void {
        ...
        for (const entry of this.poll.entitled_users_at_stop || []) {
            ...
            if (entry.vote_delegated_to_user_ids?.length) {
                entry.vote_delegated_to_user_ids.forEach(id => userIds.add(id));
            }
            if (entry.user_merged_into_id) {
                userIds.add(entry.user_merged_into_id);
            }
            if (entry.delegation_user_merged_into_ids?.length) {
                entry.delegation_user_merged_into_ids.forEach(id => {
                    if (id) {
                        userIds.add(id);
                    }
                });
                ...
                .subscribe(users => {
                    ...
                    for (const entry of this.poll.entitled_users_at_stop || []) {
                        entries.push({
                            ...
                            vote_delegated_to: entry.vote_delegated_to_user_ids?.length
                                ? entry.vote_delegated_to_user_ids
                                      .map(id => users.find(user => user.id === id))
                                      .filter(user => !!user)
                                : null,
                            vote_delegated_to_names: entry.vote_delegated_to_user_ids?.length
                                ? entry.vote_delegated_to_user_ids
                                      .map(id => users.find(user => user.id === id)?.getShortName())
                                      .filter(name => !!name)
                                      .join(`, `)
                                : null,
                            user_merged_into: entry.user_merged_into_id
                                ? `${this.translate.instant(`Old account of`)} ${users
                                      ...
                                      ?.getShortName()}`
                                : null,
                            delegation_user_merged_into: entry.delegation_user_merged_into_ids?.length
                                ? entry.delegation_user_merged_into_ids
                                      .map(id =>
                                          id
                                              ? `(${this.translate.instant(
                                                    `represented by old account of`
                                                )}) ${users.find(user => user.id === id)?.getShortName()}`
                                              : null
                                      )
                                      .filter(text => !!text)
                                      .join(`, `)
    ...
    private setLiveRegisterData(): void {
        ...
        Array.from(userIds).forEach(userId => {
            const delegatedToIds = this.userRepo.getViewModel(userId)?.vote_delegated_to_ids();
            if (delegatedToIds?.length) {
                delegatedToIds.forEach(id => delegates.add(id));
            }
        });
                    ...
                    for (const user of users || []) {
                        ...
                        entries.push({
                            ...
                            vote_delegated_to: delegateToIds?.length
                                ? delegateToIds.map(id => users.find(utmp => utmp.id === id)).filter(u => !!u)
                                : null,
                            vote_delegated_to_names: delegateToIds?.length
                                ? delegateToIds
                                      .map(id => users.find(utmp => utmp.id === id)?.getShortName())
                                      .filter(name => !!name)
                                      .join(`, `)
                                : null
                        });
    ...
    public getUsersVoteDelegation(user: ViewUser): ViewUser[] | null {
        const actorId = this.poll?.getVoteActorIdForUser?.(user.id) ?? null;
        if (actorId !== null && actorId !== user.id) {
            const actorUser = this.userRepo.getViewModel(actorId);
            return actorUser ? [actorUser] : null;
        }
